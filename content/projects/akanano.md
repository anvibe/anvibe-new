---
title: "Akanano"
slug: "akanano"
year: "07.25"
scope: "Branding, UI.UX"
timeline: "9 Weeks"
liveUrl: "https://example.com"
description: "Bliss approached us with a clear goal: grow their market share by 20% in just six months. They weren't after trends or throwaway ideas — they wanted a brand presence that felt fresh and built to last."
heroImage: "/images/projects/akanano/akanano.png"
images:
  - "https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=1920&h=1080&fit=crop"
  - "https://images.unsplash.com/photo-1551650975-87deedd944c3?w=1920&h=1080&fit=crop"
  - "https://images.unsplash.com/photo-1553028826-f4804a6dba3b?w=1920&h=1080&fit=crop"
---

We set out to create more than just a good-looking brand — we built an identity with intention. Through a complete visual refresh, paired with bold, personality-driven copy and a clear, strategic narrative, we gave Bliss a brand that didn't just turn heads, but told a story worth remembering.

## Global Brand Transformation

The transformation encompassed every touchpoint of the Bliss brand, from visual identity to brand voice. We created a cohesive system that resonated with their target audience while maintaining the innovative spirit that made Bliss unique.

